...
function doTask(event: Event): void {
  // creating a callback function to handle the event, click event in this case
  const $eventTarget = event.target as HTMLElement; // assigning the event.target to a variable (ALways!); event.target finds the element the event is triggered
